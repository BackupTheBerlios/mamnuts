...
talker_shutdown(UR_OBJECT user, const char *str, int sdboot)
{
  ...
  if (sdboot) {
    ...
    write_syslog(SYSLOG, 0, "*** REBOOT FAILED %s: %s ***\n\n", long_date(1),
                 strerror(errno));
    /* TODO: we shoudn't exit(i), but return int i, so the caller would exit(i)... */
    exit(12);
  }
...
system_details(UR_OBJECT user)
{
  ...
  static const char *const rip[] = { "OFF", "AUTO", "IDENTD" };
  ...
  if (word_count < 2) {
    write_user(user,
               "\n+----------------------------------------------------------------------------+\n");
    sprintf(text, "System Details for %s (MAMNUTS version %s)", TALKER_NAME,
            MAMNUTSVER);
