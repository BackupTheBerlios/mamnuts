 ...
 *****************************************************************************/
  ...
  for (;;) {
    ...
    for (; beat <= time(0); beat += amsys->heartbeat) {
      /* debufferiza(); */ /* USE THIS IF YOU'RE USING TMORE!!! */
      ...
      for (;;) {
        ...
        *user->buff = '\0';
        ...
        word_count = wordfind(curstr);
		    ...
		    /** buffermore 1 stuff **/ 
		    /* if ((b=find_buffer(user->socket))!=NULL) b->pd=1; */  /* use this if you're using tmore */
...
check_directories(void)
{
  ...
  for (i = 0; i < NUM_LEVELS; ++i) {
    for (j = i + 1; j < NUM_LEVELS; ++j) {
      if (!strcmp(user_level[i].name, user_level[j].name)) {
        fprintf(stderr, "Mamnuts: Level names are not unique.\n");
        boot_exit(14);
      }
    }
  }
  ...
  if (stat(dirname, &stbuf) == -1) {
    fprintf(stderr,
            "Mamnuts: Directory stat failure in check_directories().\n");
    boot_exit(15);
  }
  if (!S_ISDIR(stbuf.st_mode)) {
    fprintf(stderr, "Directory structure is incorrect.\n");
    boot_exit(16);
  }
  sprintf(dirname, "%s/%s", USERFILES, USERMAILS);
  if (stat(dirname, &stbuf) == -1) {
    fprintf(stderr,
            "Directory stat failure in check_directories().\n");
    boot_exit(15);
  }
  if (!S_ISDIR(stbuf.st_mode)) {
    fprintf(stderr, "Directory structure is incorrect.\n");
...
#undef ML_EXPAND
  ...
  for (s = fgets(line, 81, fp); s; s = fgets(line, 81, fp)) {
    ...
    if (t && !t[1]) {
      *t = '\0';
      ...
      if (!*section) {
        fprintf(stderr, "Unknown section header on line %d.\n",
                config_line);
        fclose(fp);
        boot_exit(1);
      }
      if (got_sections & BIT(section - sections)) {
        fprintf(stderr, "Unexpected %s section header on line %d.\n",
                *section, config_line);
        fclose(fp);
        boot_exit(1);
      }
    ...
    if (!section) {
      fprintf(stderr, "Section header expected on line %d.\n",
              config_line);
      fclose(fp);
      boot_exit(1);
    }
  ...
  if (!(got_sections & BIT(CONFIG_INIT))) {
    fprintf(stderr, "INIT section missing from config file.\n");
    boot_exit(1);
  }
  if (!(got_sections & BIT(CONFIG_ROOMS))) {
    fprintf(stderr, "ROOMS section missing from config file.\n");
    if (got_sections & BIT(CONFIG_TOPICS)) {
      fprintf(stderr,
              "TOPICS section must come after ROOMS section in the config file.\n");
    }
    boot_exit(1);
  }
  if (!*amsys->mport_port) {
    fprintf(stderr, "Main port number not set in config file.\n");
    boot_exit(1);
  }
#ifdef WIZPORT
  if (!*amsys->wport_port) {
    fprintf(stderr, "Wiz port number not set in config file.\n");
    boot_exit(1);
  }
  if (!strcmp(amsys->mport_port, amsys->wport_port)) {
    fprintf(stderr, "Port numbers must be unique.\n");
    boot_exit(1);
  }
#endif
#ifdef NETLINKS
  if (!*amsys->nlink_port) {
    fprintf(stderr, "Link port number not set in config file.\n");
    boot_exit(1);
  }
  if (!*amsys->verification) {
    fprintf(stderr, "Verification not set in config file.\n");
    boot_exit(1);
  }
...
#endif
  if (!room_first) {
    fprintf(stderr, "No rooms configured in config file.\n");
    boot_exit(1);
  }
  ...
  for (rm1 = room_first; rm1; rm1 = rm1->next) {
    for (i = 0; i < MAX_LINKS; ++i) {
      ...
      if (!rm2) {
        fprintf(stderr, "Room %s has undefined link label \"%s\".\n",
                rm1->name, rm1->link_label[i]);
        boot_exit(1);
      }
      if (rm2 == rm1) {
        fprintf(stderr, "Room %s cannot link to itself.\n",
                rm1->name);
        boot_exit(1);
      }
      rm1->link[i] = rm2;
  ...
  for (nl = nl_first; nl; nl = nl->next) {
    ...
    if (rm1) {
      fprintf(stderr, "Service name %s is also the name of a room.\n",
              nl->service);
      boot_exit(1);
    }
  }
  ...
  for (rm1 = room_first; rm1; rm1 = rm1->next) {
    ...
    if (!nl) {
      fprintf(stderr, "Service name %s not defined for room %s.\n",
  ...
  for (rm1 = room_first; rm1; rm1 = rm1->next) {
    ...
    fp = fopen(filename, "r");
    if (!fp) {
      fprintf(stderr, "Cannot open description file for room %s.\n",
              rm1->name);
    ...
    if (c != EOF) {
      fprintf(stderr, "Description too long for room %s.\n",
              rm1->name);
      write_syslog(SYSLOG | ERRLOG, 0,
...
#undef ML_EXPAND
  ...
  if (!*initopt) {
    fprintf(stderr, "Unknown INIT option on line %d.\n", config_line);
    boot_exit(1);
  }
  if (!*wrd[1]) {
    fprintf(stderr, "Required parameter missing on line %d.\n",
            config_line);
    boot_exit(1);
  }
  if (*wrd[2]) {
    fprintf(stderr,
            "Unexpected word following init parameter on line %d.\n",
            config_line);
    boot_exit(1);
  }
  val = atoi(wrd[1]);
  ...
  case INITOPT_MAIN_PORT:
    ...
    if (strlen(wrd[1]) >= MAXSERV) {    /* XXX: Use NI_MAXSERV */
      fprintf(stderr, "%s has Illegal port number on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  case INITOPT_MINLOGIN_LEVEL:
    {
      ...
      if (lvl == NUM_LEVELS && strcmp(wrd[1], "NONE")) {
        fprintf(stderr,
                " %s has Unknown level specifier on line %d.\n",
                *initopt, config_line);
        boot_exit(1);
      }
  ...
  case INITOPT_MESG_LIFE:
    /* message lifetime */
    if (val < 1) {
      fprintf(stderr, "%s has Illegal value on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
    amsys->mesg_life = val;
  ...
  case INITOPT_PROMPT_DEF:
    ...
    val = onoff_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "%s must be ON or OFF on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  case INITOPT_MIN_PRIVATE:
    if (val < 1) {
      fprintf(stderr,
              "%s has Number too low on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  case INITOPT_VERIFICATION:
    if (strlen(wrd[1]) > VERIFY_LEN) {
      fprintf(stderr, "%s too long on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
#ifdef NETLINKS
  ...
  case INITOPT_MESG_CHECK_TIME:
    ...
    if (wrd[1][2] != ':' || strlen(wrd[1]) > 5 || !isdigit((int) wrd[1][0])
        ...
        || !isdigit((int) wrd[1][3])
        || !isdigit((int) wrd[1][4])) {
      fprintf(stderr, "%s has Invalid value on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  case INITOPT_AUTO_CONNECT:
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "%s must be YES or NO on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
#ifdef NETLINKS
  ...
  case INITOPT_BAN_SWEARING:
    ...
    if (val == -1) {
      fprintf(stderr,
              "%s must be OFF, MIN or MAX on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  case INITOPT_CRASH_ACTION:
    if (!strcmp(wrd[1], "NONE")) {
      ...
      fprintf(stderr,
              "%s must be NONE, SHUTDOWN or REBOOT on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
    break;
  ...
  case INITOPT_RESOLVE_IP:
    ...
    if (val == -1) {
      fprintf(stderr,
              "%s must be OFF, AUTO, or IDENTD on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  case INITOPT_DEF_WARP:
    if (strlen(wrd[1]) >= ROOM_NAME_LEN + 1) {
      fprintf(stderr, "%s has Illegal room name on line %d.\n",
              *initopt, config_line);
      boot_exit(1);
    }
  ...
  default:
    fprintf(stderr, " %s is Unknown INIT option on line %d.\n",
            *initopt, config_line);
    boot_exit(1);
    break;
...
parse_rooms_section(void)
{
  ...
  if (!*wrd[3]) {
    fprintf(stderr, "Required parameter(s) missing on line %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[0]) > ROOM_NAME_LEN) {
    fprintf(stderr, "Room map name too long on line %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[1]) > ROOM_LABEL_LEN) {
    fprintf(stderr, "Room label too long on line %d.\n", config_line);
    boot_exit(1);
  }
  if (strlen(wrd[2]) > ROOM_NAME_LEN) {
    fprintf(stderr, "Room name too long on line %d.\n", config_line);
    boot_exit(1);
  }
  /* Check for duplicate label or name */
  for (room = room_first; room; room = room->next) {
    if (!strcmp(room->label, wrd[1])) {
      fprintf(stderr, "Duplicate room label on line %d.\n",
              config_line);
      boot_exit(1);
    }
    if (!strcmp(room->name, wrd[2])) {
      fprintf(stderr, "Duplicate room name on line %d.\n",
              config_line);
      boot_exit(1);
    }
  }
  ...
  for (;;) {
    ...
    if (*ptr2 == ',' && !ptr2[1]) {
      fprintf(stderr, "Missing link label on line %d.\n",
              config_line);
      boot_exit(1);
    }
    c = *ptr2;
    *ptr2 = '\0';
    if (!strcmp(ptr1, room->label)) {
      fprintf(stderr, "Room has a link to itself on line %d.\n",
              config_line);
      boot_exit(1);
    }
    ...
    if (++i >= MAX_LINKS) {
      fprintf(stderr, "Too many links on line %d.\n", config_line);
      boot_exit(1);
    }
    *ptr2 = c;
    ptr1 = ++ptr2;
  ...
  if (!strcmp(wrd[4], "BOTH")) {
    ...
    fprintf(stderr, "Unknown room access type on line %d.\n",
	    config_line);
    boot_exit(1);
  }
  ...
  if (!strcmp(wrd[5], "ACCEPT")) {
    if (*wrd[6]) {
      fprintf(stderr,
              "Unexpected word following ACCEPT keyword on line %d.\n",
              config_line);
      boot_exit(1);
    }
    room->inlink = 1;
  ...
  if (!strcmp(wrd[5], "CONNECT")) {
    if (!*wrd[6]) {
      fprintf(stderr, "External link name missing on line %d.\n",
              config_line);
      boot_exit(1);
    }
    ...
    if (*wrd[7]) {
      fprintf(stderr,
              "Unexpected word following external link name on line %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  }
  fprintf(stderr, "Unknown connection option on line %d.\n",
          config_line);
  boot_exit(1);
#endif
}
...
parse_topics_section(char *topic)
{
  ...
  if (!room) {
    fprintf(stderr, "Room does not exist on line %d.\n", config_line);
    boot_exit(1);
    return;
  }
...
parse_sites_section(void)
{
  ...
  if (!nl) {
    fprintf(stderr,
            "Memory allocation failure creating netlink on line %d.\n",
            config_line);
    boot_exit(1);
    return;
  }
  if (strlen(wrd[0]) > SERV_NAME_LEN) {
    fprintf(stderr, "Link name length too long on line %d.\n",
            config_line);
    boot_exit(1);
  }
  strcpy(nl->service, wrd[0]);
  if (strlen(wrd[1]) >= MAXHOST) {      /* XXX: Use NI_MAXHOST */
    fprintf(stderr, "Site name length too long on line %d.\n",
            config_line);
    boot_exit(1);
  }
  ...
  if (strlen(wrd[2]) >= MAXSERV) {      /* XXX: USE NI_MAXSERV */
    fprintf(stderr, "Illegal port number on line %d.\n", config_line);
    boot_exit(1);
  }
  strcpy(nl->port, wrd[2]);
  if (strlen(wrd[3]) > VERIFY_LEN) {
    fprintf(stderr, "Verification too long on line %d.\n",
            config_line);
    boot_exit(1);
  }
  ...
  if (!*wrd[4] || !strcmp(wrd[4], "ALL")) {
    ...
    fprintf(stderr, "Unknown netlink access type on line %d.\n",
            config_line);
    boot_exit(1);
  }
}
...
boot_exit(int code)
{
  ...
  case 11:
    ...
    break;
  case 12:
    perror("Failed to parse user structure");
    write_syslog(SYSLOG, 1,
                 "BOOT FAILURE: Failed to parse user structure.\n");
    break;
  case 13:
    perror("Failed to parse user commands");
  ...
  case 15:
    ...
    break;
  case 16:
    perror("Directory structure in USERFILES is incorrect");
    write_syslog(SYSLOG, 1,
                 "BOOT FAILURE: Directory structure in USERFILES is incorrect.\n");
    break;
  case 17:
    perror("Failed to create temp user structure");
    write_syslog(SYSLOG, 1,
                 "BOOT FAILURE: Failed to create temp user structure.\n");
    break;
  case 18:
    perror("Failed to parse a user structure");
    write_syslog(SYSLOG, 1,
                 "BOOT FAILURE: Failed to parse a user structure.\n");
    break;
  case 19:
    perror("Failed to open directory USERROOMS for reading");
    write_syslog(SYSLOG, 1,
                 "BOOT FAILURE: Failed to open directory USERROOMS.\n");
    break;
  case 20:
    perror("Failed to open directory in MOTDFILES for reading");
    write_syslog(SYSLOG, 1,
                 "BOOT FAILURE: Failed to open a directory MOTDFILES.\n");
    break;
  case 21:
    perror("Failed to create system object");
    write_syslog(SYSLOG, 1,
...
/* ************* UNCOMMENT THIS IF YOU WANNA USE TMORE  
  ML_ENTRY((TMORE,         "split"       )) \
*/  ML_ENTRY((COUNT,         NULL          ))
enum userdb_value
{
...
load_user_details(UR_OBJECT user)
{
  ...
  for (s = fgets(line, ARR_SIZE - 1, fp); s;
    ...
    case USERDB_GAME_SETTINGS:
      ...
      break;
	  /* IN CASE YOU WANT TO USE TMORE... */
/*  	case USERDB_TMORE:
	  	switch (wcnt) {
        default:
		    case 2:
  			  user->visualn = atoi(user_words[2]);
	  	  case 1: 
          user->visual = atoi(user_words[1]);
        case 0:
			    break;
  		}
      break;*/
...
save_user_details(UR_OBJECT user, int save_current)
{
  ...
  fprintf(fp, "%-13.13s %d %d %d %d %d %d %d %d\n",
          ...
          user->bank);
/* IF TMORE THEN INCLUDE THIS */
/*  fprintf(fp, "%-13.13s %d %d\n", userfile_options[USERDB_TMORE],
          user->visual, user->visualn);	  
*/		  
  fclose(fp);
  return 1;
}
...
process_users(void)
{
  ...
  u = create_user();
  if (!u) {
    fprintf(stderr, "Create user failure in process_users().\n");
    boot_exit(17);
    return;
  }
  ...
  if (!dirp) {
    fprintf(stderr, "Directory open failure in process_users().\n");
    destruct_user(u);
    boot_exit(12);
  }
  ...
  if (dp) {
    fprintf(stderr,
            "Cannot load userfile for \"%s\" in process_users().\n",
            u->name);
    boot_exit(18);
  }
  destruct_user(u);
...
parse_commands(void)
{
  ...
  for (com_tab = command_table; com_tab->name; ++com_tab) {
    if (!add_command((enum cmd_value) (com_tab - command_table))) {
      fprintf(stderr,
              "Memory allocation failure in parse_commands().\n");
      boot_exit(13);
    }
  }
  return;
...
void write_sock(int s, const char *str) {
  /* buffermore 2 interceptor */ 
  /* USE THIS IF YOU'RE USING TMORE */
  /*BU_OBJ b; 
  if ( (b=find_buffer(s)) ){ 
	  bufferiza(b, str,  strlen( str)  ); 
	  return; 
  } */
...
/*  buffermore 3  interceptor  */ 
/* NEEDED FOR TMORE */
/* go=0; 
 if ( (b=find_buffer(user->socket)) ) go=1; */
...
/*      if(go==1)
  ...
  if (buffpos) {
/* NEEDED FOR TMORE */
/*	if(go==1) bufferiza(b,buff,buffpos);
	else */ send(user->socket, buff, buffpos, 0);
  }
...
/*    user->visual = 0;
  ...
  case LOGIN_PROMPT:
    ...
    write_user(user, "\n\n");
    connect_user(user);
/*    inicia_visual(user); */ /* NEEDED FOR TMORE */
    return;
  }
}
...
{
  write_user(user,
             "\n~BB*** NUTS Credits :) (for Mamnuts credits, see \".help credits\") ***\n\n");
  vwrite_user(user,
...
show_version(UR_OBJECT user)
{
  ...
  write_user(user,
             "+----------------------------------------------------------------------------+\n");
  vwrite_user(user,
              "| ~FC~OLMamnuts version %-21.21s (C) Mind Booster Noori, 2006        ~RS |\n",
              MAMNUTSVER);
  vwrite_user(user,
              "| ~FC~OLBuilt %s %s~RS                                                 |\n",
              __DATE__, __TIME__);
